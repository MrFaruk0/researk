...
The source build is local-first and has no telemetry. The offline fake provider makes no network
requests. The experimental generic OpenAI-compatible adapter sends selected requests only to the
user-supplied base URL. It is not verified native support for any named provider.
...
and mathematical LaTeX falls back to visible source. No graphical math backend or system TeX
execution exists.
